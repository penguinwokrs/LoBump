...
describe('Worker Auth', () => {
    it('GET /auth/login redirects to Riot', async () => {
        ...
        expect(res.headers.get('Location')).toContain('https://auth.riotgames.com/authorize')
    ...
    it('GET /auth/callback exchanges code for token', async () => {
        ...
        global.fetch = vi.fn().mockResolvedValue({
            ok: true,
            json: async () => ({ access_token: 'mock-token', id_token: 'mock-id-token' }),
        })
        ...
        expect(body).toEqual({ access_token: 'mock-token', id_token: 'mock-id-token' })

        expect(global.fetch).toHaveBeenCalledWith('https://auth.riotgames.com/token', expect.objectContaining({
            method: 'POST',
...
describe('Session Management', () => {
    ...
    it('POST /sessions/:id/join adds a user', async () => {
        ...
        const res = await app.request(`/sessions/${sessionId}/join`, {
            method: 'POST',
